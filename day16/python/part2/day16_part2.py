...
def process(filename: str) -> int:
    ...
    for r in range(rows):
        for c in range(cols):
            tile = (r,c)
            if r == 0 and c == 0:
                maxenergy = max(maxenergy, calc_energy((tile, Dir.N)))
                maxenergy = max(maxenergy, calc_energy((tile, Dir.W)))
                continue
            if r == 0 and c == (cols-1):
                maxenergy = max(maxenergy, calc_energy((tile, Dir.N)))
                maxenergy = max(maxenergy, calc_energy((tile, Dir.E)))
                continue
            if r == (rows-1) and c == 0:
                maxenergy = max(maxenergy, calc_energy((tile, Dir.S)))
                maxenergy = max(maxenergy, calc_energy((tile, Dir.W)))
                continue
